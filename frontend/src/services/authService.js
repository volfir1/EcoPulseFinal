...
const authService = {
  ...
  googleSignIn: async () => {
    try {
      ...
      // Important: Add delay before popup to ensure direct connection to user action
      ...
      try {
        ...
        console.log('Backend response for Google auth:', apiData);
        
        // Format the user object with Firebase data
        const formattedUser = {
          id: user.uid,
        ...
        if (apiData.isDeactivated || apiData.isAutoDeactivated) {
          console.log('Account is deactivated:', apiData);
          return {
            success: false,
            isDeactivated: true,
            isAutoDeactivated: apiData.isAutoDeactivated || false,
        ...
        if (apiData.requireVerification) {
          ...
          return {
            success: false,
            requireVerification: true,
            userId: apiData.userId || apiData.user?.id,
            email: apiData.user?.email || user.email,
            message: apiData.message || "Please verify your email to complete sign-in."
          };
        }
        
        // 3. Successful authentication with existing account
        if (apiData.success) {
          console.log('Authentication successful with existing account');
          return {
            success: true,
            user: { ...formattedUser, ...apiData.user },
            token: apiData.token || apiData.user?.accessToken || token,
            message: apiData.message || "Google sign-in successful"
          };
        }
        
        // 4. Unexpected response
        console.warn('Unexpected response from Google sign-in backend:', apiData);
        ...
        // CRITICAL FIX: Handle 403 responses which are likely deactivated accounts
        if (apiError.response?.status === 403) {
          const responseData = apiError.response.data;
          console.log('Received 403 response for deactivated account:', responseData);
          
          // If the response indicates a deactivated account, handle it
          if (responseData.isDeactivated || responseData.isAutoDeactivated || 
              (responseData.message && responseData.message.includes('deactivated'))) {
            return {
              success: false,
              isDeactivated: true,
              isAutoDeactivated: responseData.isAutoDeactivated || false,
              email: responseData.email || user.email,
              message: responseData.message || "Your account has been deactivated. A reactivation link has been sent to your email."
            };
          }
        }
